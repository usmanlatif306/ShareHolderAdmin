 <!--begin::Form-->
 <form id="kt_modal_update_user_form" class="form" wire:submit="updateUser" method="POST">
     @csrf
     @method('put')
     <!--begin::Scroll-->
     <div class="d-flex flex-column scroll-y px-5 px-lg-10" id="kt_modal_add_user_scroll" data-kt-scroll="true"
         ...
         <div class="fv-row mb-7">
             ...
             <input type="password" name="password" wire:model="password"
                 ...
                 placeholder="********" />
             <small class="text-primary">{{ __('Leave empty you want to use old password') }}</small>
             @error('password')
                 <span class="invalid-feedback" role="alert">
                     <strong>{{ $message }}</strong>
                 </span>
     ...
     <div class="text-center pt-10">
         ...
         <button type="submit" class="btn btn-primary" data-kt-users-modal-action="submit">
             <span class="indicator-label">{{ __('Update User') }}</span>
         </button>
     </div>
     <!--end::Actions-->
 </form>
