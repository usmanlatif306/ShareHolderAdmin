        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="card shadow mb-4">
                <div class="card-header card-header-height d-flex align-items-center">
                    <h6 class="font-weight-bold text-primary mb-0 pb-0">{{ __('Update FAQ') }}</h6>
                </div>
                    ...
                    <form method="POST" action="{{ route('faqs.update', $faq) }}">
                        ...
                        <div class="form-group mb-6">
                            ...
                            <textarea class="form-control form-control-solid @error('answer') is-invalid @enderror" id="answer" name="answer"
                                placeholder="{{ __('Answer') }}" rows="5" required>{{ old('answer', $faq->answer) }}</textarea>
                            @error('answer')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-6">
                            ...
                            <select name="show" id="show"
                                ...
                                <option value="1" @selected(old('show', $faq->show) == '1')>{{ __('Show') }}</option>
                                <option value="0" @selected(old('show', $faq->show) == '0')>{{ __('Hide') }}</option>
                            </select>
                            @error('show')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="text-center py-10">
                            <button type="submit" class="btn btn-primary">
                                <span class="indicator-label">{{ __('Update FAQ') }}</span>
                            </button>
                        </div>
                        <!--end::Actions-->
                    </form>
