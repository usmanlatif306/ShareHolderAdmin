                    ...
                    <form method="POST" action="{{ route('categories.update', $category) }}">
                        ...
                        <div class="form-group mb-6">
                            ...
                            <textarea type="text" class="form-control form-control-solid @error('description') is-invalid @enderror"
                                id="description" placeholder="{{ __('Category Description') }}" name="description" rows="3" required>{{ old('description', $category->description) }}</textarea>
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-6">
                            ...
                            <select name="show" id="show"
                                ...
                                <option value="1" @selected(old('show', $category->show) == '1')>{{ __('Show') }}</option>
                                <option value="0" @selected(old('show', $category->show) == '0')>{{ __('Hide') }}</option>
                            </select>
                            @error('show')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
